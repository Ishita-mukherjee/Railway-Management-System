...
if(isset($_POST['search']))
{
    ...
    //$query = "SELECT * FROM `train` WHERE CONCAT(`train_id`, `train_name`, `day`, `time`,`source`,`destination`) LIKE '%".$valueToSearch."%'";
    $query = "SELECT * FROM `train` WHERE `source`='$valueToSearch' and `destination` ='$valueToSearch1' order by `train_id`";
    $search_result = filterTable($query);
    
}
 else {
    $query = "SELECT * FROM `train` order by `train_id`";
    $search_result = filterTable($query);
}
...
.train tr:nth-child(odd) {background-color: #D1F2EB;}
.train tr:hover {background-color: #f5f5f5;}
  ...
  <header class="h">
    ...
    <body style="background-image: linear-gradient(#B5F1EF,#93B8F0,#B5C6F1,#C6B5F1);">
       <nav class="menu">
            <ul class="main-menu">
                <li ><i class="fa fa-home"></i><a href="home.php"> Home</a></li>
                <li class="active"><i class="fa fa-briefcase"></i><a href="trainschedule.php"> Train Schedule</li>
        ...
        <br><br>
        <h1 align="center" style="font-size: 45px">Enter Source and Destination to Find Trains</h1>
        <form action="trainschedule.php" method="post">
            <input type="text" name="valueToSearch" placeholder="Source"><br><br>
            <input type="text" name="valueToSearch1" placeholder="Destination"><br><br>

            <input type="submit" name="search" value="Filter" style=" background: #136CA9;    color: white;box-shadow: 0 .75rem .5rem -.5rem hsl(0 50% 80%);">
            <br><br><br><br>

            <table  style="  border-radius: 10px;background-color: white;margin-left: 25%" class="train">
                <tr>
                    <th>Train Id</th>
                    <th>Train Name</th>
                    <th>Time</th>
